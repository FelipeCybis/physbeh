...
def get_place_field_coords(random=False, size=30):
    if random:
        y_mu_grid, x_mu_grid = np.random.randint(5,95,size=size), np.random.randint(5,95,size=size)
    else:
        y_mu_grid, x_mu_grid = np.meshgrid(np.arange(5,100,22.5),np.arange(5,100,22.5))
...
def get_value_from_hexagonal_grid(coord, xplus = 0, a=1/10, angle = 8 * np.pi / 18):
    ...
    re = np.cos(X*R1[0] + Y*R1[1]) + np.cos(X*R2[0] + Y*R2[1]) + np.cos(X*R3[0] + Y*R3[1])
    return re
