...
class Corner_Coords:
    def __init__(self, videopath, function_after_done, x_crop=[None, None], y_crop=[None, None]):
        ...
        self.crelease   = self.fig.canvas.mpl_connect('button_release_event', self.onrelease)
    ...
    def grab_frame(self, frame):
        ...
        try:
            self.ax.set_array(fr[self.y_crop[0]:self.y_crop[1],
                                 self.x_crop[0]:self.x_crop[1],::-1])
        except AttributeError:
            self.ax.imshow(fr[self.y_crop[0]:self.y_crop[1],
                              self.x_crop[0]:self.x_crop[1],::-1])

        self.ax.set(title='Current frame: {frame_no}\nLeft click to select a corner, right click to remove it'.format(frame_no=self.current_frame))
    ...
    def done(self, function_after_done):
        ...
        if len(self.coords_list) == 4:
            plt.close(0)
            print(f'Top left corner: {self.coords_list[0]}',
            ...
            self.ax.annotate(text='Not all corners were labeled!',
                             xy=(0.76, 0.35),
                             xycoords='figure fraction',
                             color='red',
                             bbox=dict(facecolor='white',alpha=.2, edgecolor='red'))
            self.fig.canvas.draw()
        ...
        if event.inaxes == self.ax:
            if len(self.pts_list) < 4:
                if event.button == 1: # LEFT BUTTON
                    ...
                    self.txt_list.append(self.ax.annotate(text=self.text_to_write[len(self.pts_list)-1],
                                                          xy=(event.xdata,event.ydata),
                                                          xytext=(event.xdata,event.ydata),
                                                          xycoords='data',
                                                          bbox=dict(facecolor='white',alpha=.2)))
